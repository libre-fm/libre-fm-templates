...
Template based on 'Artist' Template for GNU FM
...
{foreach from=$mbdata item=i}
  <li class="list-group-item d-flex justify-content-between align-items-center">
    <a href="/artist/{$artist->name|replace:' ':'+'|escape:'html'}/album/{$i.title|replace:' ':'+'|escape:'html'}" title="Libre.fm album page for {$i.title}" rel="bookmark">{$i.title} {if $i.country}({$i.country}){/if} {if $i.disambiguation}[{$i.disambiguation}]{/if}</a> {if $i.date}({$i.date}){/if}
  </li>
{/foreach}
</ul>
